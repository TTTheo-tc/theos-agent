...
from src.config.schema import FeishuConfig
...
class FeishuChannel(BaseChannel):
    ...
    def supports_internal_progress(self) -> bool:
        """Feishu users should only see final answers, not internal execution traces."""
        return False
    ...
    def __init__(self, config: FeishuConfig, bus: MessageBus, owner_ids: list[str] | None = None):
        ...
        self._keepalive_message_ids: OrderedDict[str, None] = OrderedDict()
        self._loop: asyncio.AbstractEventLoop | None = None

    def transform_progress_message(self, msg: OutboundMessage) -> OutboundMessage | None:
        """Suppress internals but keep one generic keepalive for long-running work."""
        ...
        source_message_id = str(msg.metadata.get("message_id") or msg.chat_id)
        if source_message_id in self._keepalive_message_ids:
            return None
        self._keepalive_message_ids[source_message_id] = None
        while len(self._keepalive_message_ids) > 1000:
            self._keepalive_message_ids.popitem(last=False)

        metadata = dict(msg.metadata or {})
        metadata.pop("_progress", None)
        metadata.pop("_progress_kind", None)
    ...
    def _upload_image_sync(self, file_path: str) -> str | None:
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error uploading image {}", file_path)
            return None
    ...
    def _upload_file_sync(self, file_path: str) -> str | None:
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error uploading file {}", file_path)
            return None
    ...
    def _download_image_sync(
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error downloading image {}", image_key)
            return None, None
    ...
    def _download_file_sync(
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error downloading {} {}", resource_type, file_key)
            return None, None
    ...
    async def _download_and_save_media(
        ...
        """
        loop = asyncio.get_running_loop()
        media_dir = Path.home() / ".theos" / "media"
    ...
    def _send_message_sync(
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error sending Feishu {} message", msg_type)
            return False

    def _send_content_with_fallback_sync(
        self, receive_id_type: str, receive_id: str, content: str
    ) -> bool:
        """Send rich interactive content, falling back to plain text on failure."""
        card = {
        ...
        if sent:
            return True
        logger.warning(
            "Interactive Feishu card send failed for {}; falling back to text message",
            receive_id,
        )
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            if msg.content and msg.content.strip():
                await loop.run_in_executor(
                    None,
                    self._send_content_with_fallback_sync,
                    receive_id_type,
                    msg.chat_id,
                    msg.content,
                )
            ...
            logger.opt(exception=True).warning("Error sending Feishu message")
    ...
    async def _on_message(self, data: "P2ImMessageReceiveV1") -> None:
        ...
        try:
            ...
            logger.opt(exception=True).warning("Error processing Feishu message")
