    ...
    async def _send_payload(
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    logger.opt(exception=True).warning("Error sending Discord message")
                else:
    ...
    async def _gateway_loop(self) -> None:
        ...
        async for raw in self._ws:
            ...
            if op == 10:
                ...
                # RECONNECT: exit loop to reconnect (will attempt RESUME)
                logger.info("Discord gateway requested reconnect")
                break
            elif op == 9:
                # INVALID_SESSION: d=true means resumable, d=false means re-identify
                resumable = payload is True
                if not resumable:
                    self._session_id = None
                    self._seq = None
                logger.warning("Discord gateway invalid session (resumable={})", resumable)
                break

    async def _resume(self) -> None:
        """Send RESUME payload to restore a previous session."""
        if not self._ws:
            return
        resume = {
            "op": 6,
        ...
        logger.info("Sending Discord RESUME (session={}, seq={})", self._session_id, self._seq)
