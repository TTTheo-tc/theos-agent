...
async def test_dedup_handles_non_json_arguments():
    class NonJson:
    ...
    provider = _make_provider(
        LLMResponse(
            content="same object",
            tool_calls=[
                ToolCallRequest(id="a", name="counting", arguments={"key": NonJson()}),
                ToolCallRequest(id="b", name="counting", arguments={"key": NonJson()}),
            ],
            finish_reason="tool_calls",
        ),
        LLMResponse(content="done", finish_reason="stop"),
