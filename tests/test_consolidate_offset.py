...
class TestSessionImmutableHistory:
    ...
    def test_get_history_preserves_all_turns_on_topic_shift(self) -> None:
        """Topic shifts must NOT discard history — the model needs full context."""
        session = Session(key="test:topic_shift")
        session.add_message("user", "你能在我的日历里添加吗？")
        session.add_message("assistant", "我现在只能帮你设 Telegram 提醒。")
    ...
    def test_get_history_preserves_follow_up(self) -> None:
        """Follow-ups retain full history (not just the last turn)."""
        session = Session(key="test:follow_up")
        session.add_message("user", "你能在我的日历里添加吗？")
        ...
        assert history[0]["content"] == "你能在我的日历里添加吗？"
        assert history[1]["role"] == "assistant"
        assert history[1]["content"] == "我现在不能直接写入系统日历。"

    def test_tool_result_soft_trim_head_tail(self) -> None:
        """Long tool results are soft-trimmed to head + tail, not hard-cut."""
        session = Session(key="test:tool_trim")
        session.add_message("user", "run the command")
        big_result = "H" * 2000 + "MIDDLE" + "T" * 2000
        session.messages.append(
            {"role": "tool", "content": big_result, "name": "shell", "tool_call_id": "t1"}
        )

        history = session.get_history(max_messages=10)
        tool_entry = [m for m in history if m.get("role") == "tool"][0]

        assert tool_entry["content"].startswith("H" * 500)
        assert tool_entry["content"].endswith("T" * 500)
        assert "[shell result trimmed]" in tool_entry["content"]
...
class TestConsolidationDeduplicationGuard:
    ...
    async def test_consolidation_guard_prevents_duplicate_tasks(self, tmp_path: Path) -> None:
        ...
        async def _fake_consolidate(
            _session, *, provider=None, model=None, memory_window=None, archive_all: bool = False
        ) -> None:
            nonlocal consolidation_calls
            consolidation_calls += 1
            await asyncio.sleep(0.05)

        loop._memory.consolidate = _fake_consolidate  # type: ignore[method-assign]
    ...
    async def test_consolidation_tasks_are_referenced(self, tmp_path: Path) -> None:
        ...
        loop._memory.consolidate = _slow_consolidate  # type: ignore[method-assign]

        msg = InboundMessage(channel="cli", sender_id="user", chat_id="test", content="hello")
        await loop._process_message(msg)

        await started.wait()
        assert len(loop._memory.consolidation_tasks) == 1, "Task must be referenced while in-flight"

        await asyncio.sleep(0.15)
        assert (
            len(loop._memory.consolidation_tasks) == 0
    ...
    async def test_new_waits_for_inflight_consolidation_and_preserves_messages(
        ...
        async def _fake_consolidate(
            sess, *, provider=None, model=None, memory_window=None, archive_all: bool = False
        ) -> bool:
    ...
    async def test_new_does_not_clear_session_when_archive_fails(self, tmp_path: Path) -> None:
        ...
        loop._memory.consolidate = _failing_consolidate  # type: ignore[method-assign]
        ...
        session_after = loop.sessions.get_or_create("cli:test")
        assert (
            len(session_after.messages) == before_count
        ), "Session must remain intact when /new archival fails"

    @pytest.mark.asyncio
    async def test_new_clears_session_when_archive_uses_fallback_summary(
        self, tmp_path: Path
    ) -> None:
        """/new should still clear the session when consolidation falls back locally."""
        from src.agent.loop import AgentLoop
        from src.bus.events import InboundMessage
        from src.bus.queue import MessageBus
        from src.config.schema import Config
        ...
        provider.chat = AsyncMock(
            return_value=LLMResponse(content="Error calling LLM", tool_calls=[])
        )
        cfg = Config()
        cfg.agents.defaults.workspace = str(tmp_path)
        cfg.agents.defaults.memory_window = 10
        ...
        assert "Archived 10 messages" in history_text
    ...
    async def test_new_cleans_up_consolidation_lock_for_invalidated_session(
        ...
        loop._memory.get_consolidation_lock(session.key)
        assert session.key in loop._memory._consolidation_locks
        ...
        loop._memory.consolidate = _ok_consolidate  # type: ignore[method-assign]
        ...
        assert response is not None
        assert "new session started" in response.content.lower()
        assert session.key not in loop._memory._consolidation_locks
